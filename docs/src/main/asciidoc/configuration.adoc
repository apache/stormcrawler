...
| textextractor.llm.prompt | - | User prompt template. Use `\{HTML}` and `\{REQUEST}` as placeholders (optional). Marker tokens of the form `<\|NAME\|>` in the template are removed from the page HTML before it is substituted.
| textextractor.llm.user_request | - | Extra user request passed to the prompt template (optional).
| textextractor.llm.text.maxlength | -1 | Maximum number of characters of extracted text, -1 for no limit. The text is taken from the `<content>` envelope of the reply if there is one, and any markup is removed.
