...
| http.custom.headers | - | Custom HTTP headers, sent to every host. A warning is logged for credential headers (see `http.credentials.headers`); use `http.basicauth.hosts` or the `protocol.set-header` metadata to send credentials to specific sites only.
