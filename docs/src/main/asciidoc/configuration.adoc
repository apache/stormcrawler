...
rules to every request the browser makes, including redirects and subresources, and aborts the
rejected ones. It cannot see the address the browser connects to: the host is resolved separately,
so short-lived DNS records or a remote browser using another resolver can lead to a different
address. WebSocket connections are not checked.
